...
class YOLOSegment(Model):
    def __init__(self):
        ...
        # your request for source code except for the Sample Code. For example, Ultralytics YOLO is licensed by
        ...
        super().__init__(
            model_file="/path/to/yolo11n-seg_imx_model/packerOut.zip",
            model_type=MODEL_TYPE.CONVERTED,
            color_format=COLOR_FORMAT.RGB,
            preserve_aspect_ratio=True,
        )

        # Default Ultralytics segment models trained on COCO with 80 classes, we provide the labels here:
        # ./modlib/models/zoo/assets
        self.labels = np.genfromtxt("/path/to/yolo11n-seg_imx_model/labels.txt", dtype=str, delimiter="\n")
