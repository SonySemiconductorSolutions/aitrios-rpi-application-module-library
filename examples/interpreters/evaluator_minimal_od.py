...
class YOLO(Model):
    def __init__(self):
        ...
        super().__init__(
            model_file=f"{MODELS}/yolo11n_imx_model/packerOut.zip",  # for IMX500 deployment
            model_type=MODEL_TYPE.CONVERTED,
            color_format=COLOR_FORMAT.RGB,
            preserve_aspect_ratio=False,
        )
        self.labels = np.genfromtxt(f"{MODELS}/yolo11n_imx_model/labels.txt", dtype=str, delimiter="\n")
        self.norm_mean = 0.0
        self.norm_std = 255.0
