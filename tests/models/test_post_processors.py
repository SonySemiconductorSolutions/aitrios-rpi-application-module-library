...
def test_pp_segment():
    ...
    assert all(1 <= idx <= 20 for idx in result.indices)
    assert len(result.indices) == len(set(result.indices))  # Ensure uniqueness
    assert len(result.indices) > 0 and len(result.indices) <= 21 - 0
